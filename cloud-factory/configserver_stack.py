...
"""Spring config server Java application CloudFormation template generator.

Usage:
    configserver_stack.py [-v] -f CONFIGURATION_FILE ENV
...
def create_name(base, az, env, config):
    return get_name_prefix(env, config) + base + "".join(az)
...
def create_key_value_tags(config, base, az, instanceNumber, env):
    tags = []
    tags.append(
        ec2.Tag("name", get_name_prefix(env, config) + "-" + base + "-zone-" + az + "-instance-" + str(instanceNumber)))
...
def create_autoscalling_tags(config, base, az, env):
    tags = []
    tags.append(autoscaling.Tag("name", get_name_prefix(env, config) + "-" + base + "-zone-" + join_zones(
        az) + "-instance-", True))
    return tags


def join_zones(zones):
    return "".join(zones)


def create_launch_config(aws_config, config, instance_type, zones, security_groups, env, java_opts):
    launch_configuration = autoscaling.LaunchConfiguration(create_lauch_config_name(zones, env, config))
    launch_configuration.EbsOptimized = config["EbsOptimized"]
    launch_configuration.IamInstanceProfile = config["IamInstanceProfile"]
    launch_configuration.ImageId = config["AMIImageId"]
    ...
    instance_config = {"stack-name": get_name_prefix(env, config), "region": aws_config["Region"], "env": env,
                       "componentType": config["Type"],
                       "component": config["Component"], "zone": "-".join(zones)}
...
def create_autoscalling_group(aws_config, config, az, security_groups, load_balancers, min_size, max_size, env):
    auto_scaling_group = autoscaling.AutoScalingGroup(create_name("ASG", az, env, config))
    auto_scaling_group.AvailabilityZones = [get_availability_zone(aws_config, zone) for zone in az]
    auto_scaling_group.LaunchConfigurationName = Ref(create_lauch_config_name(az, env, config))
    load_balancer_list = []
    ...
    auto_scaling_group.Tags = create_autoscalling_tags(config, "ASG", az, env)
    auto_scaling_group.VPCZoneIdentifier = [ aws_config["Subnets"]["Sandbox_DEV"]["PrivateSubnets"]["Sandbox_DEV_PVT_1" + zone] for zone in az]
...
def create_lauch_config_name(zones, env, config):
    return create_name("LC", join_zones(zones), env, config)
...
def create_instance(aws_config, config, az, instanceNumber, env):
    ...
    instance.ImageId = config["AMIImageId"]
...
if __name__ == '__main__':
    ...
    instance_type = Parameter(
        "InstanceType",
        Default="t2.nano",
        AllowedValues=aws_config["InstanceTypes"],
        Type="String",
    ...
    template.add_parameter(java_opts)
    ...
    autoscalling_group = create_autoscalling_group(aws_config, config, zones,
                                                   [cluster_security_group], [load_balancer], min_size, max_size,
                                                   env)
    template.add_resource(autoscalling_group)

    launch_config = create_launch_config(aws_config, config, instance_type, zones, [cluster_security_group], env, java_opts)
